...
const toggleIcon = () => {
  ...
  if (props.item.price) {
    ...
    emit('updateDrinkPrice', { price: priceValue, add: isButtonClicked.value });
  }
};
...
const emitDrinkDetails = () => {
  ...
  if (isButtonClicked.value && item && item.price) {
...
watch(isButtonClicked, emitDrinkDetails);
</script>
...
<template>
  <div class="w-[105px] h-40 outline-none border-2 border-transparent bg-card rounded-[10px] p-0.5 flex flex-col justify-center items-center">
    ...
    <button @click="toggleIcon" class="w-[30px] h-[30px] mt-2 flex justify-center items-center outline-none border-2 border-gray-400 rounded-[20px]">
      <svg v-if="isButtonClicked" class="opacity-50" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
